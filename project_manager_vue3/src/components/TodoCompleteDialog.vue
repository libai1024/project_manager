    ...
    <div class="dialog-content">
      ...
      <div class="todo-info">
        <div class="todo-info-item">
          ...
          <span class="info-value">{{ todo?.project_title }}</span>
        </div>
        <div class="todo-info-item">
          <span class="info-label">步骤：</span>
          <span class="info-value">{{ formatStepRange(todo?.step_names || []) }}</span>
        </div>
        <div class="todo-info-item">
      ...
      <div class="completion-form">
        <div class="form-header">
          <span class="form-title">完成描述</span>
        ...
        <div class="markdown-editor-wrapper">
          ...
          <el-input
            ...
            type="textarea"
            :rows="8"
              placeholder="输入完成描述，支持Markdown语法..."
          ...
          <div v-show="editorMode === 'preview'" class="preview-wrapper">
            <div class="markdown-preview" v-html="renderMarkdown(completionNote)"></div>
            <div v-if="!completionNote.trim()" class="preview-empty">
              暂无内容，切换到"编辑"标签输入内容
            </div>
          </div>
        </div>
...
const emit = defineEmits<{
  ...
  confirm: [data: { completionNote: string; files: File[]; photos: File[]; fileFolderId?: number; photoFolderId?: number }]
}>()
...
const loadFolders = async () => {
  if (!props.todo?.project_id) return
  try {
    folders.value = await attachmentFolderApi.list(props.todo.project_id)
    // 默认选择快照文件夹
...
const handleCreateFolder = async () => {
  ...
  try {
    const newFolder = await attachmentFolderApi.create(props.todo.project_id, { name: newFolderName.value.trim() })
    await loadFolders()
...
<style scoped>
.todo-complete-dialog :deep(.el-dialog__body) {
  padding: 24px;
}
...
.completion-textarea :deep(.el-textarea__inner) {
  border: none;
  box-shadow: none;
  padding: 12px 16px;
  font-size: 13px;
...
.completion-textarea :deep(.el-textarea__inner:focus) {
  box-shadow: none;
}
