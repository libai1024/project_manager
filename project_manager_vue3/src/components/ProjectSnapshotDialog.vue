    ...
    <div class="dialog-content">
      ...
      <div class="snapshot-form">
        <div class="form-header">
          <span class="form-title">快照说明</span>
        ...
        <div class="markdown-editor-wrapper">
          ...
          <div v-show="editorMode === 'edit'" class="input-wrapper">
            <el-input
              ...
              type="textarea"
              :rows="6"
              placeholder="输入快照说明，支持Markdown语法..."
          ...
          <div v-show="editorMode === 'preview'" class="preview-wrapper">
            <div class="markdown-preview" v-html="renderMarkdown(snapshotNote)"></div>
            <div v-if="!snapshotNote.trim()" class="preview-empty">
              暂无内容
            </div>
          </div>
        </div>
...
import { Plus, UploadFilled, Delete, Document } from '@element-plus/icons-vue'
...
const emit = defineEmits<{
  ...
  confirm: [data: { note: string; photos: File[]; files: File[]; fileFolderId?: number; photoFolderId?: number }]
}>()
...
const handleConfirm = () => {
  if (photoList.value.length === 0 && fileList.value.length === 0) {
    ElMessage.warning('请至少上传一张照片或一个文件')
    return
  }
...
<style scoped>
.snapshot-dialog :deep(.el-dialog__body) {
  padding: 24px;
}
...
.snapshot-textarea :deep(.el-textarea__inner) {
  border: none;
  box-shadow: none;
  padding: 12px 16px;
  font-size: 13px;
...
.file-preview-item.is-image .file-icon {
  display: none;
}

.file-preview-item.is-image .file-info {
  text-align: center;
}
