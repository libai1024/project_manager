...
from app.models.project import (
    Project, ProjectCreate, ProjectUpdate, ProjectReadWithRelations,
    ProjectStep, ProjectStepCreate, ProjectStepUpdate, ProjectStepRead,
    DEFAULT_STEPS, StepStatus
)
...
class ProjectService:
    ...
    def create_project(self, project_data: ProjectCreate, user_id: int) -> ProjectReadWithRelations:
        ...
        # 创建项目（project_repo.create 会处理 tag_ids）
        ...
        # 如果有需求文件，将它们移动到"项目需求"文件夹，并区分图片和文件
        photo_ids = []
        file_attachment_ids = []
    ...
    def get_project_with_relations(self, project_id: int) -> ProjectReadWithRelations:
        ...
        import logging
        logger = logging.getLogger(__name__)
        logger.info(f"Project {project_id} - Found {len(project_tags)} project_tag associations")
        ...
        for pt in project_tags:
            ...
            if tag:
                tag_read = TagRead.model_validate(tag)
                tags.append(tag_read)
                logger.info(f"Project {project_id} - Added tag: {tag_read.name} (id: {tag_read.id})")
            else:
                logger.warning(f"Project {project_id} - Tag with id {pt.tag_id} not found in Tag table")
        
        # 调试：记录标签加载情况
        logger.info(f"Project {project_id} - Final tags count: {len(tags)}")
        if tags:
            logger.info(f"Project {project_id} - Tags: {[t.name for t in tags]}")
        else:
            logger.warning(f"Project {project_id} - No tags found. Check if project has tag associations in projecttag table.")
        ...
        if platform:
            project_dict["platform"] = PlatformRead.model_validate(platform)
        else:
            project_dict["platform"] = None
    ...
    def list_projects(
        ...
        try:
            ...
            logger.error(f"Error in list_projects: {str(e)}", exc_info=True)
            raise
    ...
    def update_project(
        ...
        if update_data.get('is_paid') is True or update_data.get('status') == "已结账":
            # 如果actual_income为0，则使用price作为实际收入
    ...
    def settle_project(
        ...
        self.project_repo.update(project, update_data)
        logger.info(f"项目 {project_id} 手动结账，实际收入: {update_data.get('actual_income', project.actual_income)}")
    ...
    def _check_and_update_billing_status(self, project_id: int) -> None:
        ...
        if all_completed and has_billing_step and not project.is_paid:
            ...
            self.project_repo.update(project, update_data)
            logger.info(f"项目 {project_id} 自动结账，实际收入: {update_data.get('actual_income', project.actual_income)}")
