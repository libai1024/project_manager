...
from app.models.todo import TodoCreate, TodoUpdate, TodoReadWithRelations
import json
...
@router.get("/", response_model=List[TodoReadWithRelations])
async def get_todos(
...
@router.post("/", response_model=TodoReadWithRelations)
...
@router.put("/{todo_id}", response_model=TodoReadWithRelations)
...
async def get_todo_calendar(
    ...
    # 这里我们使用一个简单的方法：查询所有已结账的项目，然后按日期分组
    # 注意：这里假设结账日期就是updated_at的日期部分
